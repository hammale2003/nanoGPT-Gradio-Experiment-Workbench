...
def estimate_loss(model_local, block_size_local, micro_batch_size_eval, train_data_local, val_data_local, device_local, eval_iters_local, ctx_local, use_recompute_during_eval=False):
    ...
    for split in ['train', 'val']:
        ...
        for k in range(eval_iters_local):
            ...
            except ValueError as e: losses[k] = float('nan'); continue
            with ctx_local: _, loss = model_local(X, Y, use_recompute=use_recompute_during_eval)
            losses[k] = loss.item()
...
def run_nanoGPT_training(
    ...
    # current_script_dir is defined at the module level and is accessible here.
    # global current_script_dir # Not strictly needed if not reassigning current_script_dir locally
    ...
    # ... (other print statements for config)

    # Construct paths dynamically based on current_script_dir and dataset_name
    # Assumes 'nanoGPT' folder is inside current_script_dir (e.g. 'THIS_STUDIO')
    ...
    # Automatic Data Preparation Block
    if not (os.path.exists(train_bin_path) and os.path.exists(val_bin_path)):
        ...
        print(f"Data files for '{dataset_name}' not found in {data_dir_for_dataset}. Attempting automatic preparation...")
        
        prepare_script_path = os.path.join(data_dir_for_dataset, "prepare.py")
        if os.path.exists(prepare_script_path):
            if dataset_name not in ["shakespeare_char", "shakespeare"]: # Add more "safe" datasets if needed
            ...
            print(f"Running preparation script: {prepare_script_path}")
            # Use sys.executable to call the same Python interpreter that's running this script
            python_executable = sys.executable 
            try:
                # The cwd for prepare.py should be its own directory
                process = subprocess.run([python_executable, 'prepare.py'], cwd=data_dir_for_dataset, check=True, capture_output=True, text=True, timeout=3600) # Added timeout
                print("Preparation script stdout:"); print(process.stdout)
                print("Preparation script stderr:"); print(process.stderr)
                yield {"type": "info", "message": f"Preparation for '{dataset_name}' completed."}
                print(f"Preparation for '{dataset_name}' possibly completed. Checking files again...")
            except subprocess.CalledProcessError as e:
                error_msg = f"Error during automatic preparation of '{dataset_name}': {e.stderr}"
                print(error_msg); yield {"type": "error", "message": error_msg}; return
    ...
    # --- Actual Data Loading ---
    script_execution_dir = os.getcwd() # Where gradio_app.py is run
    ...
    except FileNotFoundError: # Should have been caught by auto-prep, but as a safeguard
    ...
    for iter_num_current_loop in range(max_iters + 1):
        ...
        if iter_num == max_iters:
             if max_iters % eval_interval_ui != 0 and max_iters > 0:
                losses = estimate_loss(model, block_size, micro_batch_size_ui, train_data, val_data, device, eval_iters_ui, ctx)
        ...
        for micro_step in range(grad_accumulation_steps_ui):
            try: X, Y = get_batch('train', block_size, micro_batch_size_ui, train_data, val_data, device)
            except ValueError as e: yield {"type": "error", "message": f"Training halted: {e}"}; return
            with ctx: logits, loss = model(X, Y, use_recompute=use_recompute_ui); loss = loss / grad_accumulation_steps_ui
            loss.backward(); accumulated_loss_for_iter += loss.item() 
        ...
        if iter_num % log_interval_ui == 0:
            yield {"type": "train_iter", "iter": iter_num, "loss": accumulated_loss_for_iter, "lr": lr,
